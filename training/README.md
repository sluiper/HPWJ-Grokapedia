# Training packs / حزم التدريب

**Governing framework:** `docs/13_Competency_Framework_Training_and_Assessment.md`  
**SSOT for numbers:** `MASTER_CONTROL_REGISTER.md`  
**Rule:** Never hard-code reaction-force constants — cite **MCR-017**.

| Code | Title | Duration | Path | Status |
|------|-------|----------|------|--------|
| ATC-HPWJ-OP-001 | Core Operator (Level 2) | 3 days | `ATC-HPWJ-OP-001/` | 0.2-draft |
| ATC-HPWJ-AWARE | Site Awareness | ½ day | `ATC-HPWJ-AWARE/` | 0.1-draft |
| ATC-HPWJ-VOC | Verification of Competency | 1 day | `ATC-HPWJ-VOC/` | 0.1-draft |

**Not yet built:** SUP, UHP, method endorsements, T3.  
